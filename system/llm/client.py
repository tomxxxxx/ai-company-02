...
from system.config import DEFAULT_MODEL, DEFAULT_MAX_TOKENS, DEFAULT_TEMPERATURE
...
class ToolUseClient:
    ...
    def __init__(self):
        ...
        if not self.api_key:
            raise ValueError("ANTHROPIC_API_KEY required. Set it in .env file.")
        ...
        try:
            ...
            raise ImportError("anthropic package required: pip install 'anthropic>=0.42.0'")
    ...
    def run_agent_loop(
        ...
        return {
            "text": "\n".join(all_text) if all_text else "[Max turns reached]",
            "tool_calls": all_tool_calls,
            "turns": turn,
            "input_tokens": total_input_tokens,
            "output_tokens": total_output_tokens,
    ...
    def _call_with_retry(self, kwargs: dict, max_retries: int = 5, initial_wait: float = 60.0):
        ...
        for attempt in range(max_retries + 1):
            try:
                ...
                is_retryable = any(x in error_str.lower() for x in [
                    "429", "rate_limit", "500", "502", "503", "529", "overloaded"
                ])
